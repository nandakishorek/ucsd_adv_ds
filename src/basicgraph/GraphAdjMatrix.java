...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
	    ...
	    List<Integer> twoHopNeighborList = new ArrayList<Integer>();
	    
        if (v >= 0 && v < adjMatrix.length) {
            
            int[][] adjSqr = new int[adjMatrix.length][adjMatrix.length];

            // square the Adj matrix
            for (int i = 0; i < adjMatrix.length; ++i) {
                for (int j = 0; j < adjMatrix.length; ++j) {
                    adjSqr[i][j] = 0;
                    for (int k = 0; k < adjMatrix.length; ++k) {
                        adjSqr[i][j] += adjMatrix[i][k] * adjMatrix[k][j];
                    }
                }
            }
            
            // get the vertices reachable by two hops
            ...
            twoHopNeighborList.addAll(twoHopNeighbors);
        }
        
		return twoHopNeighborList;
	}
